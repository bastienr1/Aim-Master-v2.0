...
export function GoalRoadmap({ goal, strategy, onNavigate }: GoalRoadmapProps) {
  ...
  if (!goal) {
    return (
      <div
        className="bg-[#2A3A47] border border-white/10 rounded-xl p-5 cursor-pointer hover:border-[#53CADC]/30 transition-colors"
        onClick={() => onNavigate('goals')}
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Target className="w-5 h-5 text-[#5A6872]" />
            <div>
              <p className="text-[#ECE8E1] text-sm font-['Rajdhani'] font-semibold">
                Set a Goal
              </p>
              <p className="text-[#5A6872] text-xs font-['Inter']">
                Unlock your personalized roadmap and daily coaching
              </p>
            </div>
          </div>
          <ArrowRight className="w-4 h-4 text-[#5A6872]" />
        </div>
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-['Inter'] uppercase tracking-wider text-[#5A6872]">
            Active Mission
          </span>
        </div>
        {daysLeft !== null && (
      ...
      {strategy.gates.length > 0 ? (
        <div className="flex flex-wrap gap-2 mb-3">
          {strategy.gates.slice(0, 3).map((gate) => (
            <div
              key={gate.name}
              className="bg-[#0F1923] rounded-lg px-3 py-2 flex items-center gap-2"
            >
              <div
                className="w-1.5 h-1.5 rounded-full"
                style={{ backgroundColor: RANK_COLORS[gate.rank] || '#5A6872' }}
              />
              <div>
                <span className="text-[#ECE8E1] text-xs font-['Inter'] font-medium">
                  {gate.name}
                </span>
                <span className="text-[#5A6872] text-[10px] font-['JetBrains_Mono'] ml-1.5">
                  {gate.percentile}%
                </span>
                <span
          ...
          {strategy.gates.length > 3 && (
            <div className="bg-[#0F1923] rounded-lg px-3 py-2 flex items-center">
              <span className="text-[#5A6872] text-[10px] font-['Inter']">
        ...
        <div className="flex items-center gap-2 mb-3 bg-[#3DD598]/10 rounded-lg px-3 py-2">
